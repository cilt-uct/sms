...
public class FakeDataPreload {
   ...
   public SmsItem item1 = new SmsItem("Item 1", USER_ID, LOCATION1_ID, Boolean.FALSE, new Date());
   public SmsItem item2 = new SmsItem("Item 2", USER_ID, LOCATION2_ID, Boolean.FALSE, new Date());
   public SmsItem accessitem = new SmsItem("new access item", ACCESS_USER_ID, LOCATION1_ID, Boolean.FALSE, new Date());
   public SmsItem maintitem = new SmsItem("New maint item title", MAINT_USER_ID, LOCATION1_ID, Boolean.FALSE, new Date());
   public SmsItem adminitem = new SmsItem("New admin item title", ADMIN_USER_ID, LOCATION2_ID, Boolean.FALSE, new Date());
   ...
   public void preloadTestData() {
       ...
       * of the type SmsItem, then it runs the dao save method on them:
       * dao.findById(SmsItem.class, item1.getId());
       * This does the same thing as writing a bunch of these:
       * dao.save(item1);
       */
      Field[] fields = this.getClass().getDeclaredFields();
      for (Field field : fields) {
         if (field.getType().equals(SmsItem.class)) {
            try {
               dao.save((SmsItem)field.get(this));
            } catch (Exception e) {
   ...
   public void reloadTestData() {
      /*
       * This iterates over the fields in FakeDataPreload and finds all the ones which are
       * of the type SmsItem, then it sets the field equal to the method:
       * dao.findById(SmsItem.class, item1.getId());
       * This does the same thing as writing a bunch of these:
       * item1 = (SmsItem) dao.findById(SmsItem.class, item1.getId());
       */
      Field[] fields = this.getClass().getDeclaredFields();
      for (Field field : fields) {
         if (field.getType().equals(SmsItem.class)) {
            try {
               field.set(this, dao.findById(SmsItem.class, ((SmsItem)field.get(this)).getId()));
            } catch (Exception e) {
               throw new RuntimeException(e.getMessage(), e);
            }
         }
