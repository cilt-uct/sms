	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		if (viewparams != null) {
			...
			if (statusParams != null && statusParams.id != null) {
				...
				UIBranchContainer status = UIBranchContainer.make(tofill,
						"status:");
				String statusCode = smsTask.getStatusCode();
				UILink.make(status, "sms-status",
						statusUtils.getStatusIcon(statusCode)).decorate(
						new UIAlternativeTextDecorator(statusUtils
								.getStatusFullName(statusCode)));
				UIOutput.make(status, "sms-status-title", statusUtils
						.getStatusFullName(statusCode));

				UIMessage.make(tofill, "sms-sent", "ui.sent.sms.completed",
				...
				UIMessage.make(tofill, "recipient-header",
						"ui.sent.sms.header.recipients");
				UIMessage.make(tofill, "username-header",
						"ui.sent.sms.header.username");
				UIMessage.make(tofill, "status-header",
