		...
		if (! externalLogic.isUserAllowedInLocation(currentUserId, ExternalLogic.SMS_SEND, currentSiteId )){
			...
			if (! hasAccount && ! isEditing ){
				...
				if ( smsParams.id != null && ! "".equals(smsParams.id) ){
					smsTask = smsTaskLogic.getSmsTask(Long.parseLong(smsParams.id));
					//Top links
					userNavBarRenderer.makeNavBar(tofill, "navIntraTool:", null, currentUserId, currentUserId);	
					UIMessage.make(tofill, "sms-header", "ui.edit.sms.header");
				}else{
					//Top links
					userNavBarRenderer.makeNavBar(tofill, "navIntraTool:", VIEW_ID, currentUserId, currentUserId);
				...
				//textarea
				UIInput.make(form, "messageBody", null, smsTask.getId() == null ? null : smsTask.getMessageBody());
